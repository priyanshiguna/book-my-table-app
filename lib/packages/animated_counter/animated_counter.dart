...
class AnimatedFlipCounter extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < digits.length - fractionDigits; i++) {
      final digit = _SingleDigitFlipCounter(key: ValueKey(digits.length - i), value: digits[i].toDouble(), duration: duration, curve: curve, size: prototypeDigit.size, color: color, padding: padding);
    ...
    return DefaultTextStyle.merge(
      ...
      child: Row(
        ...
        children: [
          ...
          for (int i = digits.length - fractionDigits; i < digits.length; i++) _SingleDigitFlipCounter(key: ValueKey("decimal$i"), value: digits[i].toDouble(), duration: duration, curve: curve, size: prototypeDigit.size, color: color, padding: padding),
...
class _SingleDigitFlipCounter extends StatelessWidget {
  ...
  const _SingleDigitFlipCounter({super.key, required this.value, required this.duration, required this.curve, required this.size, required this.color, required this.padding});
...
/// Advance version of [AnimatedFlipCounter]
class AnimatedFlipCounter extends StatelessWidget {
  final num value;
  final Duration duration;
  final Curve curve;
  ...
  String formatCompact(num number, {int fractionDigits = 1}) {
    if (number < 1000) {
      return number.toStringAsFixed(fractionDigits).replaceAll(RegExp(r"([.]*0+)(?!.*\d)"), "");
    } else if (number < 1000000) {
      final result = number / 1000;
      return '${result.toStringAsFixed(fractionDigits).replaceAll(RegExp(r"([.]*0+)(?!.*\d)"), "")}k';
    } else if (number < 1000000000) {
      final result = number / 1000000;
      return '${result.toStringAsFixed(fractionDigits).replaceAll(RegExp(r"([.]*0+)(?!.*\d)"), "")}M';
    } else {
      final result = number / 1000000000;
      return '${result.toStringAsFixed(fractionDigits).replaceAll(RegExp(r"([.]*0+)(?!.*\d)"), "")}B';
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final formatted = formatCompact(value, fractionDigits: fractionDigits);
    ...
    for (int i = 0; i < chars.length; i++) {
      ...
      if (RegExp(r'\d').hasMatch(ch)) {
        widgets.add(_SingleDigitFlipCounter(
          key: ValueKey('digit$i'),
          value: double.parse(ch),
          duration: duration,
          curve: curve,
        ...
        // Non-digit characters: suffixes or decimal points
        widgets.add(Text(ch, style: style));
      }
    }
    ...
    return DefaultTextStyle.merge(
      ...
      child: Row(
        ...
        children: [
          ...
          ClipRect(
            child: TweenAnimationBuilder(
              // Animate the negative sign (-) appear/disappear
              duration: duration,
              tween: Tween(end: value < 0 ? 1.0 : 0.0),
